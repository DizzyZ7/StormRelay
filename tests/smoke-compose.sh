...
curl -fsS --fail-with-body http://localhost:8080/readyz
curl -fsS --fail-with-body http://localhost:8081/readyz
AUTH='Authorization: Bearer local-development-only-change-me'
curl -fsS --fail-with-body -H "$AUTH" http://localhost:8080/api/v1/version

# Register and exercise the side-effect-free process plugin over the versioned protocol.
PLUGIN=$(curl -fsS --fail-with-body -H "$AUTH" -H 'Content-Type: application/json' \
...
curl -fsS --fail-with-body -H "$AUTH" -H 'Content-Type: application/json' \
...
RUNBOOK_JSON=$(curl -fsS --fail-with-body -H "$AUTH" -H 'Content-Type: application/yaml' --data-binary @examples/runbooks/approval-demo.yaml http://localhost:8080/api/v1/runbooks)
...
test -n "$RUNBOOK_ID"
EXEC_JSON=$(curl -fsS --fail-with-body -H "$AUTH" -H 'Content-Type: application/json' -d '{"dry_run":false}' "http://localhost:8080/api/v1/runbooks/$RUNBOOK_ID/run")
...
for i in $(seq 1 40); do
  APPROVALS=$(curl -fsS --fail-with-body -H "$AUTH" "http://localhost:8080/api/v1/approvals?execution_id=$EXEC_ID")
...
test -n "$APPROVAL_ID"
curl -fsS --fail-with-body -H "$AUTH" -H 'Content-Type: application/json' -d '{"reason":"compose smoke"}' "http://localhost:8080/api/v1/approvals/$APPROVAL_ID/approve" >/dev/null

FINAL=''
for i in $(seq 1 40); do
  EXECUTION=$(curl -fsS --fail-with-body -H "$AUTH" "http://localhost:8080/api/v1/executions/$EXEC_ID")

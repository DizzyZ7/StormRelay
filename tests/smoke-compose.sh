...
wait_for_tempo_span "$EVENT_TRACE_ID" stormrelay.db.complete_delivery "$TMP_DIR/event-trace.json"
for span_name in stormrelay.event.publish stormrelay.event.process stormrelay.db.process_event stormrelay.event.deduplicate stormrelay.incident.correlate stormrelay.policy.evaluate stormrelay.notification.enqueue stormrelay.notification.deliver stormrelay.db.complete_delivery; do

...
func TestPersistedNotificationContinuesEventTrace(t *testing.T) {
	recorder, cleanupTracing := installPlatformSpanRecorder(t)
	...
	bus, err := messaging.Connect(natsURL(), "TRACE_PLATFORM_"+suffix, "stormrelay.trace.platform."+suffix, "trace_platform_consumer_"+suffix, "trace-platform-integration")
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(bus.Close)
	...
	rootCtx, root := otel.Tracer("stormrelay.integration").Start(context.Background(), "trace-platform.ingress", trace.WithSpanKind(trace.SpanKindServer))
	rootSpanID := root.SpanContext().SpanID()
	rootTraceID := root.SpanContext().TraceID()
	if err := bus.PublishEvent(rootCtx, event, "trace-platform-message-"+suffix); err != nil {
		root.End()
		t.Fatal(err)
	}
	root.End()
	...
	required := []string{
		"trace-platform.ingress",
		"stormrelay.event.publish",
		"stormrelay.event.process",
		"stormrelay.db.process_event",
		...
		"stormrelay.db.commit_event",
		"stormrelay.event.ack",
		"stormrelay.notification.deliver",
		"stormrelay.db.complete_delivery",
	}
	spans := waitForPlatformTraceSpans(t, recorder, rootTraceID, required, 12*time.Second)
	byName := map[string]sdktrace.ReadOnlySpan{}
	for _, span := range spans {
		if _, exists := byName[span.Name()]; !exists {
			byName[span.Name()] = span
		}
		assertPlatformSafeSpanData(t, span, secretMarker)
	}

	assertPlatformParent(t, byName, "stormrelay.event.publish", rootSpanID)
	assertPlatformParent(t, byName, "stormrelay.event.process", byName["stormrelay.event.publish"].SpanContext().SpanID())
	assertPlatformParent(t, byName, "stormrelay.db.process_event", byName["stormrelay.event.process"].SpanContext().SpanID())
	for _, child := range []string{
		...
		assertPlatformParent(t, byName, child, byName["stormrelay.db.process_event"].SpanContext().SpanID())
	}
	assertPlatformParent(t, byName, "stormrelay.event.ack", byName["stormrelay.event.process"].SpanContext().SpanID())
	assertPlatformParent(t, byName, "stormrelay.notification.deliver", byName["stormrelay.notification.enqueue"].SpanContext().SpanID())
	assertPlatformParent(t, byName, "stormrelay.db.complete_delivery", byName["stormrelay.notification.deliver"].SpanContext().SpanID())
}

func installPlatformSpanRecorder(t *testing.T) (*tracetest.SpanRecorder, func()) {
	t.Helper()
...
func waitForPlatformTraceSpans(t *testing.T, recorder *tracetest.SpanRecorder, traceID trace.TraceID, required []string, timeout time.Duration) []sdktrace.ReadOnlySpan {
	t.Helper()
	deadline := time.Now().Add(timeout)
	for time.Now().Before(deadline) {
		spans := platformTraceSpans(recorder.Ended(), traceID)
	...
	names := []string{}
	for _, span := range platformTraceSpans(recorder.Ended(), traceID) {
...
func platformTraceSpans(spans []sdktrace.ReadOnlySpan, traceID trace.TraceID) []sdktrace.ReadOnlySpan {
...
func assertPlatformParent(t *testing.T, spans map[string]sdktrace.ReadOnlySpan, child string, want trace.SpanID) {
	t.Helper()
	span := spans[child]
	if span == nil {
...
func assertPlatformSafeSpanData(t *testing.T, span sdktrace.ReadOnlySpan, secretMarker string) {
	t.Helper()
